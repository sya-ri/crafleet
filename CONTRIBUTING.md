...
Push verification runs on `master` and `v*` tags; feature branches use pull-request verification to avoid running the same suite twice. `workflow_dispatch` remains available for branches without a pull request. Formatting, types, architecture, release notes, and coverage thresholds run on the Node.js 24.11.1 development baseline; all supported development Node versions still run the complete unit/integration suite, build, and package verification.

Integration tests run independently of real server E2E. Windows CI splits integration files across two isolated runners with `--shard=1/2` and `--shard=2/2`, retaining `--no-file-parallelism` within each shard because real PowerShell ACL helpers can exhaust their process deadlines when files compete on one runner. Explicit concurrent operations and lock contention within each test still run; production timeouts and permission checks are unchanged. Linux and macOS each run the complete integration suite. Every integration shard is required by `All supported environments`.

The existing pnpm store cache is supplemented by a cache of `artifacts/fixtures/cache`, keyed by OS, architecture, and both fixture locks. A lock change may restore older content-addressed downloads, but the builder always verifies SHA-256 and size and downloads missing artifacts. It still rebuilds plugins and checks reproducibility on every run. Generated fixture manifests with absolute paths, compiled plugins, server runtimes, worlds, and test homes are never cached.
