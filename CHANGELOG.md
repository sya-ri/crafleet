...
- `config.files` in `crafleet.yaml` configures runtime-relative discovery globs, with ordered exclusions and reinclusion. Omission retains the existing standard candidates, an explicit list replaces them, and an empty list disables discovery of new files. `config capture --initial` uses the same rules; ordinary diff/capture continue to operate on managed files.
